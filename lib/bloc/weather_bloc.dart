...
class WeatherBloc extends BaseBloc {
  ...
  Observable<WeatherModel> get weatherStream =>
      Observable(_weatherController.stream);

  Observable<String> get backgroundStream =>
      Observable(_backgroundController.stream);
  ...
  Future<WeatherModel> requestWeather(String id) async {
    var resp = await Application.http.getRequest(WeatherApi.WEATHER_STATUS,
  ...
  Future<String> requestBackground() async {
    var resp = await Application.http.getRequest<String>(
        WeatherApi.WEATHER_BACKGROUND,
        error: (msg) => _logger.log(msg, 'background'));
    return resp == null || resp.data == null
        ? WeatherApi.DEFAULT_BACKGROUND
        : resp.data;
  }

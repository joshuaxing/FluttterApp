 ...
 * @LastEditTime: 2020-09-02 18:00:18
 * @LastEditors: your name
 * @Description: In User Settings Edit
 * @FilePath: \weather_app\lib\bloc\provinces_bloc.dart
 */
import 'package:weather_app/bloc/bloc_provider.dart';
import 'package:weather_app/configs/application.dart';
import 'package:weather_app/model/district_model.dart';
import 'package:weather_app/model/province_model.dart';
...
class ProvincesBloc extends BaseBloc {
  ...
  Observable<List<ProvinceModel>> get cityStream =>
      Observable(_citiesController.stream);
  ...
  Future<List<ProvinceModel>> requestAllProvinces() async {
    ...
    return resp == null || resp.data == null
        ? []
        : ProvinceModel.fromMapList(resp.data);
  }

  /// 请求省内城市
  Future<List<ProvinceModel>> requestAllCitiesInProvince(String proid) async {
    var resp = await Application.http.getRequest(
        '${WeatherApi.WEATHER_PROVINCE}/$proid',
  ...
  /// 请求市内的区
  Future<List<DistrictModel>> requestAllDistrictsInCity(
      String proid, String cityid) async {
    var resp = await Application.http.getRequest(
        '${WeatherApi.WEATHER_PROVINCE}/$proid/$cityid',
        error: (msg) => _logger.log(msg, 'district'));
    return resp == null || resp.data == null
        ? []
        : DistrictModel.fromMapList(resp.data);
  }

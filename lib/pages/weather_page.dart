 ...
 * @LastEditTime: 2020-09-02 18:01:44
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \weather_app\lib\pages\weather_page.dart
 */
import 'dart:convert';
import 'dart:io';
    ...
    return WillPopScope(
      onWillPop: () {
        showDialog(
            context: context,
            builder: (context) =>
                AlertDialog(content: Text('是否退出当前 App'), actions: <Widget>[
                  FlatButton(
                      child: Text('好的走你~'),
                      onPressed: () {
                        SystemNavigator.pop();
            ...
            builder: (_, AsyncSnapshot<String> themeSnapshot) => Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    image: DecorationImage(
                        image: NetworkImage(themeSnapshot.data),
                        fit: BoxFit.cover),
                  ),
                  child: StreamBuilder(
                                      ...
                                      child: StreamBuilder(
                                        ...
                                        builder: (_, headerSnapshot) =>
                                          ...
                                          slivers: <Widget>[
                                            ...
                                            // 生活建议
                                            SliverToBoxAdapter(
                                                child: LifeSuggestions(
                                                    snapshot: snapshot))
                                          ],
                                        ),
                                      ),
                                      onRefresh: () async {
                                        _bloc.requestWeather(city).then(
                                            (w) => _bloc.updateWeather(w));
                                        return null;
                                      }),
                                )),
                )),
          ...
          (_, index) => Container(
              ...
              child: index == 0
                  ...
                  : Row(
                      ...
                      children: <Widget>[
                        Text(_forecastList[index - 1].date,
                            style:
                                TextStyle(fontSize: 16.0, color: Colors.white)),
                        Expanded(
                            child: Center(
                                child: Text(_forecastList[index - 1].cond.txtD,
                                    style: TextStyle(
...
class LifeSuggestions extends StatelessWidget {
  ...
  Widget _suggestionWidget(String content) => Padding(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          _suggestionWidget(
              '舒适度：${_suggestion.comf.brf}\n${_suggestion.comf.txt}'),
          _suggestionWidget(
              '洗车指数：${_suggestion.cw.brf}\n${_suggestion.cw.txt}'),
          _suggestionWidget(
              '运动指数：${_suggestion.sport.brf}\n${_suggestion.sport.txt}'),
        ],
      ),
    );
  }
